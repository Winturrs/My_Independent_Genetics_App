...
console.log(childEar)
  ...
  return(
    ...
    />     
            <TextAnimationFadeIn value={"If the two parent blood types are " + parentsBloodType + " then the childs blood type will most likely be one of these:" + childBlood } delay={10} duration={200} style={{color: '#00ffff', fontSize:20}} />
<Text> </Text>
            <TextAnimationFadeIn value={"If the two parent blood types are " + parentsBloodTypeEnd + " then the childs blood type will most likely be one of these:" + childBloodEnd } delay={15} duration={200} style={{color: '#7a49a5', fontSize:20}} />
            <Text> </Text>
             <TextAnimationFadeIn value={"If the two parent eye types are " + colorType + " then the childs eye color will most likely be one of these:" + childEyeColor } delay={20} duration={200} style={{color: '#3cb371', fontSize:20}} />
             <Text> </Text>
  <TextAnimationFadeIn value={"If the two parent hair colors are " + parentHair + " then the childs hair color will most likely be one of these:" + childHairColor } delay={25} duration={200} style={{color: '#C80000', fontSize:20}} />
  <Text> </Text>
             <TextAnimationFadeIn value={"If the two parent earlobe types are " + parentEar + " then the childs earlobe type will most likely be one of these:" + childEar } delay={30} duration={200} style={{color: '#ffa500', fontSize:20}} />

<TouchableOpacity onPress ={()=>{this.props.navigation.navigate('GeneticsScreen')}} style = {{position:'absolute', top:320}}>

   <Image source = {require('../dna_icon.png')} style ={{height:70,width:70,position:'absolute',top:320,left:90}}/>
   </TouchableOpacity>
  <TouchableOpacity onPress ={()=>{this.props.navigation.navigate('Home')}} style = {{position:'absolute', top:320}}>
  <Image source = {require("../R.png")} style ={{height: 50, width: 110, position: "absolute", top: 320,left: 180}}/>
